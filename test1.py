...
def get_page_link(state, page_no):
    return f"https://www.realestate.com.au/buy/in-{state}/list-{page_no}"
...
for i in range(1, 5):
    ...
    property_page = driver.find_elements(By.XPATH, '//a[contains(@class, "details-link residential-card__details-link")]')

    for prty in property_page:
        link = prty.get_attribute("href")
        print(link)
        f = open("rea_prop_links.txt", "a")
        f.write(link + "\n")
        f.close()
